...
import com.ttn.ttnplayer.util.SECURITY_LEVEL_L1
...
class TTNPlayerFragment : PlayerBaseFragment<FragmentTtnplayerBinding>(), TtnPlayerListener, PlayerListener.TimeChangeListener {
     ...
     * Time sliding
     */
    protected val shemaroohandler = Handler(Looper.getMainLooper())

    /**
     * Slide
     */
    protected val shemaroomeRunner = object : Runnable {

        override fun run() {
            //playback event with analytics
            playerModel?.let {
                if(PROVIDER_SHEMAROO.equals(it.getProvider(), true)) {
                    shemaroohandler.postDelayed(this, 20000)
                    viewModel.callShemaroomeAnalyticsPlayEvent(
                        it,
                        "playback_time",
                        "playback",
                        20
                ...
                else if(PROVIDER_LIONSGATE.equals(playerModel?.getProvider(), true)){
                    shemaroohandler.postDelayed(this, 20000)
                    viewModel.callLionsgateAnalyticsPlayEvent(
                        it,
                        subtitle,
                        subtitleLang,
                        watchedDuration
                ...
                else if (PROVIDER_PLANET_MARATHI.equals(playerModel?.getProvider(), true)) {
                    shemaroohandler.postDelayed(this, 20000)
                    viewModel.callPlanetMarathiAnalyticsPlayEvent(
                        it,
                        "playback"
                    )
                }
    ...
    override fun initializePlayerHelper(savedInstanceState: Bundle?) {
        ...
        e("DetailViewModel", "playerModel!!.getLA_URL() : ${playerModel!!.getLA_URL()}")
        ...
        if (PROVIDER_SHEMAROO.equals(playerModel?.getProvider(), true)
            || PROVIDER_VOOTKIDS.equals(playerModel?.getProvider(), true)
            || PROVIDER_VOOTSELECT.equals(playerModel?.getProvider(), true)
        ) {
...
//        requireActivity().window.setFlags(WindowManager.LayoutParams.FLAG_SECURE, WindowManager.LayoutParams.FLAG_SECURE)
    ...
    private fun handleTvodPurchasePlayback() {
        if(RENTAL.equals(playerModel?.getContractName(), true))
            if(detailsResponse?.data?.metaDetails?.showCase == false &&
                detailsResponse?.data?.metaDetails?.isPlaybackStarted == false)
                playerModel?.getContentId()?.let { viewModel.callPurchaseExpiryAPI(it) }
    ...
    override fun setObserver() {
        ...
        viewModel.getPlaybackExpiry().observe(viewLifecycleOwner, Observer { it ->
            e("getPlaybackExpiry", "value : $it")
            it.getContentIfNotHandled()?.let {
                detailsResponse?.data?.metaDetails?.isPlaybackStarted = true
                detailsResponse?.data?.metaDetails?.purchaseExpiry = it.data?.purchaseExpiry ?: detailsResponse?.data?.metaDetails?.purchaseExpiry ?: ""
                updatePurchseExpiryView()
            }
        })
    ...
    private val TAG = TTNPlayerFragment::class.java.name
    ...
    override fun onResume() {
        ...
        if (isPlayerStarted)
            try {
                updateAnalyticHandler()
                isPausedState = false
                /*requireActivity().window.setFlags(
                    WindowManager.LayoutParams.FLAG_SECURE,
                    WindowManager.LayoutParams.FLAG_SECURE
                )*/
                e(TAG, "inside onResume isPlayerPaused:$isPlayerPaused, " +
...
//                playerBinding.playerView.findViewById<TextView>(R.id.tv_ffwd)?.show()
//            } else {
//                playerBinding.playerView.findViewById<TextView>(R.id.tv_ffwd)?.hide()
//            }
            }
        }
    }
    ...
    private fun updateAnalyticHandler() {
        shemaroohandler.removeCallbacks(shemaroomeRunner)
        if(PROVIDER_SHEMAROO.equals(playerModel?.getProvider(), true)||
            PROVIDER_PLANET_MARATHI.equals(playerModel?.getProvider(), true)||
            PROVIDER_LIONSGATE.equals(playerModel?.getProvider(), true))
            shemaroohandler.postDelayed(shemaroomeRunner, 20000)
    }
...
//        releasePlayer()
    ...
    override fun onTTNPlayerError(e: ExoPlaybackException?) {
        e?.let { handlePlayerError(it) }
    }

    private fun handlePlayerError(exception: ExoPlaybackException) {
        e("TTNPlayerFragment","inside handlePlayerError l3Support:$l3Support," +
                " playerModel?.getProvider() : ${playerModel?.getProvider()}" +
                "cause: ${exception.cause?.message} securityLevel: $securityLevel")
        val message = exception.cause?.message?:""
        if(PROVIDER_VOOTSELECT.equals(playerModel?.getProvider(), true) && l3Support == 0
            && SECURITY_LEVEL_L1.equals(securityLevel, true)
            && message.contains("Crypto key not available")){
            e("TTNPlayerFragment","inside handlePlayerError: change security level")
            l3Support++
            videoQuality = null
            releasePlayer()
            //change security level from l1 to l3 only
            securityLevel = SECURITY_LEVEL_L3
...
//            createAndPrepareTTNPlayer(null)
    ...
    override fun releaseTtnPlayerCalled() {
        ...
        d(TAG, "releaseExoPlayerCalled playDuration: $playDuration , watchedDuration: $watchedDuration")
        if(System.currentTimeMillis() - watchedDuration > 1000) {
            if(PROVIDER_EPIC_ON.equals(playerModel?.getProvider(), true) ||
                PROVIDER_DOCU_BAY.equals(playerModel?.getProvider(), true))
                playerModel?.let { viewModel.callEpiconAnalyticsPlayEvent(it, watchedDuration.div(1000)) }
        }
        ...
        view?.setOnClickListener { view ->
            ...
            if (`object` is String) {
                ...
                if (tag.equals(FWD_TAG, true)) {
                    ...
                    createAndPrepareTTNPlayer(null)
//                    isPlayerEnded = false
                    /*ttnPlayerHelper?.playVideo(
                        playerModel?.getPlaybackUrl()!!,
                        playerModel?.getDrmLicenseUrl()
                    )*/
...
//            val watchDuration : Long = (ttnPlayerHelper?.currentPosition ?: 0) / 1000
    ...
    private fun setTimer(contentItem: ContentItem) {
        ...
        viewModel.timer = object : CountDownTimer(NEXT_EPISODE_TIMER_MS, 1000) {
            override fun onTick(millisUntilFinished: Long) {
                ...
                e("setTimer","inside onTick $secs secs")
                context?.let {
    ...
    private fun playEpisode(contentItem: ContentItem) {
        e("setTimer","inside playEpisode isPlayerPaused$isPlayerPaused")
        viewModel.timer?.cancel()
        ...
        /*playAuthType = JWTToken same as TataPlay content with epids */
        if (PROVIDER_TATA_SKY.equals(detailsResponse?.data?.metaDetails?.provider, true)) {
            contentItem.playerDetails?.offerIds?.epids?.let {
        ...
        else if (PROVIDER_CHAUPAL.equals(contentItem.provider, true)) {
            viewModel.fetchChaupalContentPlayback(contentItem.providerContentId, contentType, false)
        }
        else if (PROVIDER_SHEMAROO.equals(contentItem.provider, true)) {
        ...
        else if (PROVIDER_HOICHOI.equals(contentItem.provider, true)) {
            val token = viewModel.getToken(contentItem.id)
            playerModel?.setPlaybackUrl(contentItem.playerDetails?.playUrl)
            playerModel?.setPlaybackSubtitleUrl(contentItem.subtitlePlayUrl)
            if(playerModel?.getPlaybackUrl().isNullOrEmpty())
                navigateToPlayer(playerModel!!)
            else if (!token.isNullOrEmpty()) {
                playHoiChoiContent(playerModel?.getPlaybackUrl(), token)
            } else {
                val request = HoichoiRequest(partner = "hoichoi")
                viewModel.fetchHoichoiPlaybackUrl(request,contentItem.id)
            }
        }
        else if (PROVIDER_PLANET_MARATHI.equals(contentItem.provider, true)) {
            viewModel.fetchPlanetMarathiPlayUrl(
                playerModel?.getProviderContentId()?:"",
                playerModel?.getContentType()?:""
            )
        }
        else if (PROVIDER_LIONSGATE.equals(contentItem.provider, true)) {
            viewModel.fetchLionsgateToken(playerModel?.getPlaybackUrl())
        }
    ...
    override fun onPlayerResponse(response: Response) {
        ...
        if (response.code != 200) {
            ...
            try {
                e("TTNPlayerFragment", "inside onPlayerResponse try block ${response.body}")
                if (response.body != null) {
                    val data: BaseResponse? =
                        Gson().fromJson(response.body!!.string(), BaseResponse::class.java)

                    if(RENTAL.equals(contentItem.contractName, ignoreCase = true)
                        && jwtFailureAttempts == 0 &&
                        response.code == LAErrorCode.UNAUTHORISED
                        && data?.code == ControlErrorCodes.SESSION_FAILURE
                    ){
                        jwtFailureAttempts++
                        releasePlayer()
                        playerModel?.setResumeTime(((ttnPlayerHelper?.currentPosition ?: 0) / 1000))
                ...
                e("TTNPlayerFragment", "inside onPlayerResponse catch block ${e.message}")
                e.printStackTrace()
                updateErrorModel(
                    viewModel.VIDEO_UNAVAILABLE_TITLE,
                    "license error",
    ...
    fun onUserLeaveHint() {
        e("LandingActivity","inside onUserLeaveHint ttnplayer")
        if(isPlayerStarted && startPlayTime !=0L &&
            (PROVIDER_EPIC_ON.equals(playerModel?.getProvider(), true) ||
                    PROVIDER_DOCU_BAY.equals(playerModel?.getProvider(), true))
        ){
            val playedDuration = (System.currentTimeMillis() - startPlayTime)
            if(TimeUnit.MILLISECONDS.toSeconds(playedDuration) >= 1)
                playerModel?.let { viewModel.callEpiconAnalyticsPlayEvent(it, playedDuration.div(1000))}
        }
    }
