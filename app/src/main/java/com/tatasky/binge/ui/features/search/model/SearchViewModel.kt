...
class SearchViewModel @Inject constructor(val useCase: CommonUseCase, val sharedPrefs: PrefsRepo) :
    ...
    private val _searchUserPreferredLanguageFilterResponse = MutableLiveData<SingleEvent<List<ContentItem>>>()
    private val _searchUserPreferredGenreFilterResponse = MutableLiveData<SingleEvent<List<ContentItem>>>()

    private var _lastSearchedQuery = ""
//    private val _searchFiltersResponse = MutableLiveData<SingleEvent<RecommendationResponse>>()

    private val _searchFiltersResponse = MutableLiveData<SingleEvent<HashMap<String,RecommendationResponse>>>()
    ...
    private val _browseByLangOrGenreRail = MutableLiveData<SingleEvent<MutableList<HomeResponse.Items>?>>()
    val getBrowseByLangOrGenreResponse: LiveData<SingleEvent<MutableList<HomeResponse.Items>?>> = _browseByLangOrGenreRail
    ...
    fun getUserPreferredGenreFilter() : MutableLiveData<SingleEvent<List<ContentItem>>> = _searchUserPreferredGenreFilterResponse

    fun getUserPreferredLanguageFilter() : MutableLiveData<SingleEvent<List<ContentItem>>> = _searchUserPreferredLanguageFilterResponse
    ...
    val mFilterClick = object : CommonDTOClickListener {
        override fun onSubItemClick(
            ...
            if(iSectionType.equals("APPS",true)){
                _appRailItemClick.postValue(SingleEvent(
                    ContentItemTransitions(
                        iListItem,
                        extras,
                        iSectionType
                ...
                _filterItem.postValue(
                    SingleEvent(
                        FilterItemTransitions(
                            ...
                            iSectionType,
                            extras,
                            iListItem.backgroundImage?:"",
                            iListItem.image,railTitle,
                            iListItem.pageType)
                    )
                )
            }
    ...
    fun setClickedSuggestion(suggestion: ContentItem) {
        suggestion.source = EDITORIAL
        suggestion.origin = SOURCE_SEARCH
        _clickedItem.postValue(
            SingleEvent(
                ContentItemTransitions(
                    suggestion,
                    FragmentNavigatorExtras(), EDITORIAL
                )
            )
        )
    }
...
//        mAdapter.setTotalItemsCount(railResponse.data?.totalCount ?: 0)
        ...
        railResponse.data?.continuePagination = false//itemCount > 0 && totalItems < totalSearchCount
        if (searchPageOffset == 0) {
    ...
    fun resetSearchResults() {
        ...
        mAdapter.updateList(mutableListOf(), false)
...
//        var placeHolder = "UC_BBG"
        ...
        val request = TARequest(placeHolder, "10",
...
//        useCase.getSearchKeystrokeResult(intent,"", genre, language, true, searchPageOffset, 20)
            ...
            .subscribeWith(object : CallbackWrapper<RecommendationResponse>() {
                override fun onSuccessResponse(t: RecommendationResponse) {
                    ...
                    if (t.data == null) {
                        ...
                        if (searchPageOffset == 0 && !(contentType.equals(PROVIDER_GAMEZOP,true))) {
                            t.data!!.contentItem.addAll(0,
                                removeDuplicateContent(t.data!!.contentItem,
    ...
    fun updateLangGenreList(railResponse: RecommendationResponse, isSquareLayout:Boolean = false) {
        setProgressing(false)
        mAdapter.removeLoading()
        mAdapter.setTotalItemsCount(railResponse.data?.totalCount ?: 0)
        mAdapter.setAutoUpdate(true)
