...
import com.tatasky.binge.databinding.LayoutSettingsItemBinding
...
class SettingsFragment : BaseFragment<FragmentSettingsBinding, SettingsViewModel>() {
    ...
    private fun showHideClickedItemArrow(clickedSettingsItemId: Int) {
        /** Keeping it to local variable with lazy init
         * to get the latest binding on each call
         * as on rotation getting binding uninitialized fatal exception
         */
        val setOfSettingsItemViewBinding by lazy {
            setOf(
                binding.viewEditProfile,
                binding.viewContentLanguage,
                binding.viewDeviceManagement,
                binding.viewParentalPin,
                binding.viewSwitchAccount,
                binding.viewTransactionHistory
            )
        }
        // By default Edit profile arrow will be enabled from XML Layout file of tablet
        // as that is the first screen which will be visible to the user
        setOfSettingsItemViewBinding.forEach { settingsItemBinding ->
            settingsItemBinding.menuArrowVisible =
                clickedSettingsItemId == settingsItemBinding.root.id
        }
    }
...
//                    moveToHome()
            ...
            it.getContentIfNotHandled()?.let {
                when (it) {
                    AUTO_PLAY_TRAILER_SETTINGS_KEY -> {
                        ...
                        binding.viewAutoTrailer.switchView.isChecked =
                            !binding.viewAutoTrailer.switchView.isChecked
                    ...
                    WATCH_NOTI_SETTINGS_KEY -> {
                        ...
                        binding.viewNotificationSettings.switchView.isChecked =
                            !binding.viewNotificationSettings.switchView.isChecked
                        sideMenuDrawerAnalytics.trackNotificationSettingsChanged(
                            PARA_WATCH_NOTIFICATION,
                            if (binding.viewNotificationSettings.switchView.isChecked) YES else NO
                        )
                    }
                }
            }
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        userData.apply {
            email = savedUserData?.emailId
            firstName = savedUserData?.firstName
            lastName = savedUserData?.lastName
            rmn = savedUserData?.rmn
            image = savedUserData?.imageUrl
        }

        sendEditProfileData()
    }

    private fun sendEditProfileData() {
        if (context?.let { isTablet(it) } == true) {
            childFragment = EditProfileFragment()
            args = Bundle()
            args.putParcelable("profileModel", userData)
    ...
    override fun toBeCalledOnce() {
        ...
        binding.viewNotificationSettings.switchView.setOnTouchListener { v, event ->
...
//            }
    ...
    private fun checkForAccountRefreshComplete() {
        ...
        if (refreshCallsCount == 0) {
            ...
            activity?.let {
                if (!isTablet(it)) {
                    layoutParams =
                        ConstraintLayout.LayoutParams(/*Custom view's parent is CL so using CL layout params*/
            ...
            sharedPrefs.getConfigResponse()?.data?.config?.accountRefresh?.let {
                showToast(
                    context,
                    it, layoutParam = layoutParams
                )
            }
        }
    }
    ...
    private fun handleBottomStaticMenuItemClicks() {
        val logoutVerbiages = sharedPrefs.getConfigResponse()?.data?.config

        binding.tvLogOut.setOnClickListener {
            val dialogModel = DialogModel(
                false,
                R.drawable.ic_logout,
                logoutVerbiages?.logout?.header,
                logoutVerbiages?.logout?.logout,
                logoutVerbiages?.notNow,
                logoutVerbiages?.logout?.subHeader
            )
    ...
    private fun uiChanges() {
        ...
        activity?.let {
            if (isTablet(it)) {
                ...
                {
                    binding.apply {
                        viewEditProfile.tvSettingsItem.textSize = tabletTextSize(it)
                        viewContentLanguage.tvSettingsItem.textSize = tabletTextSize(it)
                        viewDeviceManagement.tvSettingsItem.textSize = tabletTextSize(it)
                        viewParentalPin.tvSettingsItem.textSize = tabletTextSize(it)
                        viewSwitchAccount.tvSettingsItem.textSize = tabletTextSize(it)
                        viewAutoTrailer.tvSettingsItem.textSize = tabletTextSize(it)
                        viewNotificationSettings.tvSettingsItem.textSize = tabletTextSize(it)
                        viewTransactionHistory.tvSettingsItem.textSize = tabletTextSize(it)
                        tvAccountRefresh.textSize = tabletTextSize(it)
                        tvLogOut.textSize = tabletTextSize(it)
                    }
                }
                ...
                width =
                    if (this.resources.configuration.orientation == Configuration.ORIENTATION_PORTRAIT) {
                        if (is7InchTablet(it)) {
                            (getDisplayMatics().widthPixels * 0.48).toInt()
                        } else {
                            (getDisplayMatics().widthPixels * 0.46).toInt()
                        }
                    } else {
                        if (is7InchTablet(it)) {
                            (getDisplayMatics().widthPixels * 0.4).toInt()
                        } else {
                            (getDisplayMatics().widthPixels * 0.35).toInt()
                        }
        ...
        val layoutParams = binding.setting?.layoutParams as ConstraintLayout.LayoutParams
            ...
            if (context?.let { it1 -> isTablet(it1) } == true) {
                when (v.id) {
                    R.id.view_parental_pin -> {
                        showHideClickedItemArrow(v.id)
                        loginAnalytics.trackParentalControlClick()
                        childFragment = ParentalControlSettingsFragment()
                        transaction =
                            childFragmentManager.beginTransaction()
                        transaction.setTransition(FragmentTransaction.TRANSIT_FRAGMENT_FADE)
                        transaction.replace(R.id.SidePaneContainer, childFragment).commit()
                    ...
                    R.id.view_content_language -> {
                        showHideClickedItemArrow(v.id)
                        childFragment = ContentLanguageFragment()
                        args = Bundle()
                        args.putString("title", contentLangTitle)
                        childFragment.arguments = args
                        transaction =
                            childFragmentManager.beginTransaction()
                        transaction.setTransition(FragmentTransaction.TRANSIT_FRAGMENT_FADE)
                    ...
                    R.id.view_device_management -> {
                        showHideClickedItemArrow(v.id)
                        childFragment = DeviceListFragment()
                        transaction =
                            childFragmentManager.beginTransaction()
                        transaction.setTransition(FragmentTransaction.TRANSIT_FRAGMENT_FADE)
                        transaction.replace(R.id.SidePaneContainer, childFragment).commit()
                    ...
                    R.id.view_transaction_history -> {
                        showHideClickedItemArrow(v.id)
                        childFragment = TransactionHistoryFragment()
                        args = Bundle()
                        args.putString(
                            KEY_ALIAS_NAME,
                    ...
                    R.id.view_switch_account -> {
                        ...
                        if (sharedPrefs.isParentalPinExists()
                            ...
                            childFragment = SwitchAccountFragment()
                            transaction =
                                childFragmentManager.beginTransaction()
                            transaction.setTransition(FragmentTransaction.TRANSIT_FRAGMENT_FADE)
                            transaction.replace(R.id.SidePaneContainer, childFragment).commit()
                ...
                when (v.id) {
                    ...
                    R.id.view_content_language -> {
                        findNavController().navigateSafe(
                            SettingsFragmentDirections.actionSettingsFragmentToContentLanguage(
                                contentLangTitle
                            )
                        )
                    }
